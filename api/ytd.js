...
module.exports = async (req, res) => {
    ...
    try {
        const finnhubUrl = `${FINNHUB_BASE_URL}/stock/metric?symbol=${symbol}&metric=price&token=${FINNHUB_API_KEY}`;
        const response = await axios.get(finnhubUrl);
        res.status(200).json(response.data);

    } catch (error) {
        console.error('Proxy YTD Error:', error.message);
        res.status(500).send('Service Error fetching YTD.');
    }
};
